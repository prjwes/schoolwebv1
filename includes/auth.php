...
function isRoleAtCapacity($role) {
    $limits = [
        'Admin' => 3,
        'HOI' => 1,
        'DHOI' => 1
    ...
    $current_count = countUsersByRole($role);
    return $current_count >= $limits[$role];
}
...
function areAllCriticalRolesFilled() {
    ...
    $admin_count = countUsersByRole('Admin');
    $hoi_count = countUsersByRole('HOI');
    $dhoi_count = countUsersByRole('DHOI');
    
    return $admin_count >= 3 && $hoi_count >= 1 && $dhoi_count >= 1;
}
...
function addUserByAdmin($username, $email, $full_name, $role, $created_by) {
    ...
    if (in_array($role, ['Admin', 'HOI', 'DHOI'])) {
        if (isRoleAtCapacity($role)) {
            $limits = ['Admin' => 3, 'HOI' => 1, 'DHOI' => 1];
            $limit = $limits[$role];
            return ['success' => false, 'error' => "Cannot add more $role users. Maximum limit is $limit."];
        }
    }
