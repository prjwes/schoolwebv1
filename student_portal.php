...
// Get exam results - with error checking
$exam_query = "SELECT e.id as exam_id, e.exam_name, e.exam_type, e.total_marks, e.exam_date, e.grade
               FROM exams e 
...
// Get report cards
$rc_query = "SELECT * FROM report_cards WHERE student_id = ? ORDER BY academic_year DESC, term DESC LIMIT 10";
$stmt = $conn->prepare($rc_query);
if ($stmt) {
    $stmt->bind_param("i", $student['id']);
    $stmt->execute();
...
// Get clubs
$club_query = "SELECT c.club_name, c.id, cm.role FROM clubs c 
               JOIN club_members cm ON c.id = cm.club_id 
               WHERE cm.student_id = ? LIMIT 20";
            ...
            <div class="table-container" style="margin-bottom: 24px;">
                <div class="table-header">
                    <div style="display: flex; justify-content: space-between; align-items: center;">
                        ...
                        <div>
                            <a href="student_details.php?id=<?php echo $student['id']; ?>" class="btn btn-sm" style="background: #339af0; color: white; margin-right: 8px;">Edit Details</a>
                        </div>
                    </div>
                </div>
                <div style="padding: 24px;">
                    ...
                    <p><strong>Parent/Guardian Name:</strong> <?php echo htmlspecialchars($student['parent_name'] ?? 'Not provided'); ?></p>
                    ...
                    <p><strong>Date of Birth:</strong> <?php echo $student['date_of_birth'] ? formatDate($student['date_of_birth']) : 'Not provided'; ?></p>
                </div>
            </div>
