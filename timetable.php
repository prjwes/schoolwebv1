...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_teacher'])) {
    ...
    if (empty($assignments)) {
        ...
        try {
            ...
            $stmt = $conn->prepare("DELETE FROM timetable_teachers WHERE id = ?");
            $stmt->bind_param("i", $teacher_id);
            $stmt->execute();
            $stmt->close();
            
            // Get original teacher name from database
            $stmt = $conn->prepare("SELECT teacher_name FROM timetable_teachers WHERE id = ? LIMIT 1");
            $stmt->bind_param("i", $teacher_id);
            $stmt->execute();
            $result = $stmt->get_result();
            $teacher_row = $result->fetch_assoc();
            $stmt->close();
            $teacher_name = $teacher_row['teacher_name'] ?? sanitize($_POST['teacher_name']);
            ...
            foreach ($assignments as $assignment) {
                ...
                if (count($parts) === 2) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO timetable_teachers (teacher_name, teacher_code, grade, subject) VALUES (?, ?, ?, ?)");
                    $stmt->bind_param("ssss", $teacher_name, $teacher_code, $grade, $subject);
                    $stmt->execute();
                    $stmt->close();
                }
            ...
            $conn->commit();
            header('Location: timetable.php?success=Teacher updated');
            exit();
        } catch (Exception $e) {
            $conn->rollback();
...
if (isset($_GET['delete_teacher'])) {
    ...
    $stmt->bind_param("i", $teacher_id);
    if ($stmt->execute()) {
        header('Location: timetable.php?success=Teacher deleted');
        exit();
    }
    $stmt->close();
}
...
<head>
    ...
    <title>Timetable Management - SCHOOL PORTAL</title>
        ...
        <main class="main-content">
                        ...
                        <tbody>
                            <?php foreach ($teachers as $index => $teacher): ?>
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-sm" onclick="openEditTeacher('<?php echo $teacher['id']; ?>', '<?php echo $teacher['teacher_name']; ?>', '<?php echo $teacher['teacher_code']; ?>')">Edit</button>
                                        <button type="button" class="btn btn-sm btn-danger" onclick="deleteTeacher('<?php echo $teacher['id']; ?>')">Delete</button>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                            ...
                            <?php if (empty($teachers)): ?>
                                <tr>
                                    <td colspan="5" style="text-align: center;">No teachers added yet</td>
                                </tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
            ...
            <div class="table-container">
                <div class="table-header">
                    ...
                    <h3><a href="https://prjwes.github.io/timetable/" style="color: green;text-decoration: underline;">Timetable Generator 2</a></h3>
                </div>
                <div class="table-responsive">
                    <table>
                        <thead>
    ...
    <div id="editModal" class="modal">
        <div class="modal-content">
            ...
            <form method="POST" action="timetable.php">
                ...
                <div class="form-group">
                    <label>Teacher Name *</label>
                    <input type="text" id="editTeacherName" name="teacher_name" required>
                </div>
                <div class="form-group">
                    <label>Teacher Code *</label>
                    <input type="text" id="editTeacherCode" name="teacher_code" required maxlength="10">
                </div>
                <div class="form-group">
                    <label>Select Subject/Grade Combinations *</label>
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(200px, 1fr)); gap: 10px; margin-top: 10px;">
                ...
                <button type="submit" name="edit_teacher" class="btn btn-primary">Update Teacher Assignments</button>
            </form>
        </div>
    </div>

    <script>
    function openEditTeacher(teacherId, teacherName, teacherCode) {
